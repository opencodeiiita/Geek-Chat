...
io.on('connection', socket => {
  ...
  socket.emit('announcement', formatMessages('GeekChat Bot', 'Welcome to GeekChat!'));
  socket.to(room).emit('announcement', formatMessages('GeekChat Bot', `${usrnm} entered the chat!`));
  ...
  socket.on('disconnect', () => {
    ...
    if (userIndex !== -1) {
      ...
      io.in(user.room).emit('announcement', formatMessages('GeekChat Bot', `${user.name} disconnected.`));
    }
  });
  ...
  socket.on('chatMessage', messageObject => {
    ...
    if (user) {
      ...
      io.in(user.room).emit('message', formatMessages(user.name, messageObject.msg, messageObject.userID));
    }
  });
});
